...
function ClassCard({ cls, showPrice }: { cls: CourseClass; showPrice: boolean }) {
  return (
    <Card className="h-full bg-card border border-border hover:border-primary/30 transition-all duration-500 overflow-hidden">
      ...
      <CardHeader>
        <div className="flex items-center justify-between mb-2">
          {showPrice ? (
            <span className="text-2xl font-bold text-gradient-gold">{cls.priceLabel}</span>
          ) : (
        ...
        <CardTitle className="text-lg">{cls.title}: {cls.subtitle}</CardTitle>
        <CardDescription>{cls.description}</CardDescription>
      </CardHeader>
      <CardContent>
        <div className="flex flex-col gap-1 text-sm text-muted-foreground mb-4">
          <span className="flex items-center gap-1">
            <Clock className="w-4 h-4 flex-shrink-0" /> {cls.duration} — {cls.format}
          </span>
        </div>
        <div className="space-y-1 mb-4">
          {cls.topics.map((topic) => (
            <p key={topic} className="text-sm text-muted-foreground flex items-center gap-2">
        ...
        {showPrice && (
          <>
            <p className="text-xs text-muted-foreground mb-4">前提: {cls.prerequisites}</p>
            <Link
              to="/contact"
    ...
    <PageLayout>
            ...
            {progressionSteps.map((step, i) => (
              ...
              >
                <div className="text-center px-4 py-3 rounded-lg bg-card border border-border min-w-[140px]">
                  ...
                  {step.free && (
                    <Badge variant="secondary" className="mt-2 text-xs">無料</Badge>
                  )}
                </div>
      ...
      <section className="cinematic-section bg-cinema-gradient">
        <div className="container mx-auto px-6">
          ...
          >
            ...
            <p className="text-muted-foreground">まずは無料クラスでMayaの世界を体験してください</p>
          </motion.div>
      ...
      <section className="cinematic-section">
        <div className="container mx-auto px-6">
          ...
          >
            ...
            <p className="text-muted-foreground">ライブセッションでプロから直接学べる本格コース</p>
          </motion.div>
      ...
      <section className="cinematic-section bg-cinema-gradient">
        <div className="container mx-auto px-6">
          ...
          >
            <h2 className="text-2xl md:text-3xl font-bold mb-4">
              CG Online Academyの<span className="text-gradient-gold">特徴</span>
            </h2>
          </motion.div>
